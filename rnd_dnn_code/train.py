import torch
import time
from helpers import early_stopping, get_device, one_hot_embedding
...
def train_model(model=None,
    ...
    valid_accuracy = Accuracy(num_classes = num_classes,task='multiclass').to(device)
    ...
    df.to_csv(path_or_buf=str(results_file_path)+str(condition_name)+'_losses_accuracy_original.csv')
    df1.to_csv(path_or_buf=str(results_file_path +str(condition_name)+'_train_results_original.csv'))
    df2.to_csv(path_or_buf=str(results_file_path)+str(condition_name)+'_valid_results_original.csv')
